...
// Optimized parsePrice function - Giữ nguyên từ code cũ, đã test hoạt động tốt
function parsePrice(priceText) {
    if (!priceText || priceText === 'Không có' || priceText === 'Không hiển thị') return null;
...
// Get products from Firestore - Giữ nguyên, thêm check empty
async function getProductsFromFirestore(db) {
    try {
        console.log('📊 Fetching products from Firestore...');
        const snapshot = await db.collection('products').get();
...
// DMX Scraper function - Port exact từ code cũ, thêm retry 3 lần để fix timeout
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            await page.goto(url, { 
                ...
                timeout: 60000  // Tăng timeout để fix lỗi
            });
            await delay(5000);
...
// WellHome Scraper function - Port exact, thêm retry
...
// QuangHanh Scraper function - Port exact, thêm retry
...
// Main scraping function với Firestore integration - Port full từ code cũ, thêm retry
async function performScraping(isScheduled = false) {
    console.log('==== BẮT ĐẦU CÀO GIÁ TỪ CẢ 3 WEBSITE ====');
